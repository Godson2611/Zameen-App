const data = {
  popular: {
    heading: "Popular Apartment in Mysuru",
    SelecteButtons: [{ name: "2 BHK" }, { name: "3 BHK" }],
    moreBtn: {
      name: "See More",
      iconClass: "fa-solid fa-arrow-right ms-2",
    },
    cards: [
      {
        img: "https://s3-alpha-sig.figma.com/img/9061/1c0b/740cfe7923702cf61e5b5a614f947e22?Expires=1721606400&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=hQqJRWAsdaoiMGAWIjHHyolvgeo5iluApJ6UXMrjgixu2OZtPuGqNr1~IrpKCtAKx9EKJmHs9zZkb2zXB0kyvAgc2aXYAG3UhffaPv0elEshdMswuGMg4IL27NOfer4DGuYfUsXPApztRnukZlrudUfsiIYLVSDXnceHqep2I7XT~rWoylIYMO7ILtHuBmr~fq08zLftPqGYso7oVNYTn5HUzJV4UXY8ZsnFbGDZPHwBOrNXzNvTSBE7grpaNDmrhnQ7~JGWtS09KXbZAeqdcenCqmZwKKpjsq7ll-AoUa3T1Q4mfDzMq066JVJpgJ-OlZxX4vzLEej9T4oqQkSj1A__",
        price: "70 Lac",
        detail: "2 BHK flat in Vijayanagar, Mysuru",
        location:
          "2Nd Floor, Dejgow Building, Kannada Sahithya Parishath Rd, Mysuru - 570017",
        features: [
          {
            name: "2400Sqft",
            iconClass: "fa-regular fa-square blue px-2",
          },
          {
            name: "3 wash",
            iconClass: "fa-solid fa-sink blue px-2",
          },
          {
            name: "Furnished",
            iconClass: "fa-solid fa-couch blue px-2",
          },
        ],
        date: "27/Sep/2021",
        btn: {
          name: "View Details",
          className: "btn btn-primary",
        },
      },
      {
        img: "https://s3-alpha-sig.figma.com/img/9061/1c0b/740cfe7923702cf61e5b5a614f947e22?Expires=1721606400&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=hQqJRWAsdaoiMGAWIjHHyolvgeo5iluApJ6UXMrjgixu2OZtPuGqNr1~IrpKCtAKx9EKJmHs9zZkb2zXB0kyvAgc2aXYAG3UhffaPv0elEshdMswuGMg4IL27NOfer4DGuYfUsXPApztRnukZlrudUfsiIYLVSDXnceHqep2I7XT~rWoylIYMO7ILtHuBmr~fq08zLftPqGYso7oVNYTn5HUzJV4UXY8ZsnFbGDZPHwBOrNXzNvTSBE7grpaNDmrhnQ7~JGWtS09KXbZAeqdcenCqmZwKKpjsq7ll-AoUa3T1Q4mfDzMq066JVJpgJ-OlZxX4vzLEej9T4oqQkSj1A__",
        price: "70 Lac",
        detail: "3 BHK flat in Vijayanagar, Mysuru",
        location:
          "2Nd Floor, Dejgow Building, Kannada Sahithya Parishath Rd, Mysuru - 570017",
        features: [
          {
            name: "2400Sqft",
            iconClass: "fa-regular fa-square blue px-2",
          },
          {
            name: "3 wash",
            iconClass: "fa-solid fa-sink blue px-2",
          },
          {
            name: "Furnished",
            iconClass: "fa-solid fa-couch blue px-2",
          },
        ],
        date: "27/Sep/2021",
        btn: {
          name: "View Details",
          className: "btn btn-primary",
        },
      },
    ],
  },
  project: {
    heading: "Projects in Mysuru",
    title:
      "Lorem ipsum dolor sit amet consectetur. Libero lobortis donec velit habitant. Lectus aliquam adipiscing id auctor et cursus lectus dictum. Morbi ac.",
    cards: [
      {
        img: "https://s3-alpha-sig.figma.com/img/dc65/be1e/1125f6772ed4f467da95ee102e169fbe?Expires=1721606400&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=mtdeACLApMHED6RLNl96rNsGswSj4q5LgJCc74IdH11jprLb0G-YXwp-4pBJXODo~8y2KdKqKlMkBgPi416ZUa6ffX1xw6j-tAibOjE8r7d1m-yJwMrlVKTcIJAW3W1FL2D18oxIySdkiXr15kykADl-wedLuebprWirXLaCJkhsLcPtr4YI9OcJmReNVnEusFIA2Y1yR~CdJGfs2Q7~45yAaxY0Xn9JsWWkmYQGaJ12xcTL0muVWxqDSdd~cImkETnpVHBM8Dg472vn2iUrUD9~l172YCQzBmmOsDN2A-PPEPisB0EGrSe3nszf30CToJ-2RVm~ayV~nC8T8XxziA__",
        price: "₹ 70 Lac",
        detail: "2BHK flat in Vijayanagar, Mysuru",
        location:
          "2Nd Floor, Dejgow Building, Kannada Sahithya Parishath Rd, Mysuru - 570017",
        badge: {
          name: "Success",
          class: "badge bg__green",
        },
      },
    ],
  },
  location: {
    heading: "Plots in Prime Location",
    SelecteButtons: [{ name: "1200 Sqft" }, { name: "2400 Sqft" }],
    moreBtn: {
      name: "See More",
      iconClass: "fa-solid fa-arrow-right ms-2",
    },
    cards: [
      {
        img: "https://s3-alpha-sig.figma.com/img/9061/1c0b/740cfe7923702cf61e5b5a614f947e22?Expires=1721606400&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=hQqJRWAsdaoiMGAWIjHHyolvgeo5iluApJ6UXMrjgixu2OZtPuGqNr1~IrpKCtAKx9EKJmHs9zZkb2zXB0kyvAgc2aXYAG3UhffaPv0elEshdMswuGMg4IL27NOfer4DGuYfUsXPApztRnukZlrudUfsiIYLVSDXnceHqep2I7XT~rWoylIYMO7ILtHuBmr~fq08zLftPqGYso7oVNYTn5HUzJV4UXY8ZsnFbGDZPHwBOrNXzNvTSBE7grpaNDmrhnQ7~JGWtS09KXbZAeqdcenCqmZwKKpjsq7ll-AoUa3T1Q4mfDzMq066JVJpgJ-OlZxX4vzLEej9T4oqQkSj1A__",
        price: "70 Lac",
        detail: "2 BHK flat in Vijayanagar, Mysuru",
        location:
          "2Nd Floor, Dejgow Building, Kannada Sahithya Parishath Rd, Mysuru - 570017",
        features: [
          {
            name: "2400Sqft",
            iconClass: "fa-regular fa-square blue px-2",
          },
          {
            name: "3 wash",
            iconClass: "fa-solid fa-sink blue px-2",
          },
          {
            name: "Furnished",
            iconClass: "fa-solid fa-couch blue px-2",
          },
        ],
        date: "27/Sep/2021",
        btn: {
          name: "View Details",
          className: "btn btn-primary",
        },
      },
      {
        img: "https://s3-alpha-sig.figma.com/img/9061/1c0b/740cfe7923702cf61e5b5a614f947e22?Expires=1721606400&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=hQqJRWAsdaoiMGAWIjHHyolvgeo5iluApJ6UXMrjgixu2OZtPuGqNr1~IrpKCtAKx9EKJmHs9zZkb2zXB0kyvAgc2aXYAG3UhffaPv0elEshdMswuGMg4IL27NOfer4DGuYfUsXPApztRnukZlrudUfsiIYLVSDXnceHqep2I7XT~rWoylIYMO7ILtHuBmr~fq08zLftPqGYso7oVNYTn5HUzJV4UXY8ZsnFbGDZPHwBOrNXzNvTSBE7grpaNDmrhnQ7~JGWtS09KXbZAeqdcenCqmZwKKpjsq7ll-AoUa3T1Q4mfDzMq066JVJpgJ-OlZxX4vzLEej9T4oqQkSj1A__",
        price: "70 Lac",
        detail: "3 BHK flat in Vijayanagar, Mysuru",
        location:
          "2Nd Floor, Dejgow Building, Kannada Sahithya Parishath Rd, Mysuru - 570017",
        features: [
          {
            name: "2400Sqft",
            iconClass: "fa-regular fa-square blue px-2",
          },
          {
            name: "3 wash",
            iconClass: "fa-solid fa-sink blue px-2",
          },
          {
            name: "Furnished",
            iconClass: "fa-solid fa-couch blue px-2",
          },
        ],
        date: "27/Sep/2021",
        btn: {
          name: "View Details",
          className: "btn btn-primary",
        },
      },
    ],
  },
  residential: {
    heading: "Residential Houses & Villas in Prime Locations",
    subHeading: "Select number of bedrooms here",
    SelecteButtons: [
      { name: "2 BHK" },
      { name: "+ 2 BHK" },
      { name: "+ 2 BHK" },
      { name: "+ 3 BHK" },
      { name: "+ 4 BHK" },
      { name: "+ 2 BHK" },
    ],
    moreBtn: {
      name: "See More",
      iconClass: "fa-solid fa-arrow-right ms-2",
    },
    cards: [
      {
        img: "https://s3-alpha-sig.figma.com/img/9061/1c0b/740cfe7923702cf61e5b5a614f947e22?Expires=1721606400&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=hQqJRWAsdaoiMGAWIjHHyolvgeo5iluApJ6UXMrjgixu2OZtPuGqNr1~IrpKCtAKx9EKJmHs9zZkb2zXB0kyvAgc2aXYAG3UhffaPv0elEshdMswuGMg4IL27NOfer4DGuYfUsXPApztRnukZlrudUfsiIYLVSDXnceHqep2I7XT~rWoylIYMO7ILtHuBmr~fq08zLftPqGYso7oVNYTn5HUzJV4UXY8ZsnFbGDZPHwBOrNXzNvTSBE7grpaNDmrhnQ7~JGWtS09KXbZAeqdcenCqmZwKKpjsq7ll-AoUa3T1Q4mfDzMq066JVJpgJ-OlZxX4vzLEej9T4oqQkSj1A__",
        price: "70 Lac",
        detail: "2 BHK flat in Vijayanagar, Mysuru",
        location:
          "2Nd Floor, Dejgow Building, Kannada Sahithya Parishath Rd, Mysuru - 570017",
        features: [
          {
            name: "2400Sqft",
            iconClass: "fa-regular fa-square blue px-2",
          },
          {
            name: "3 wash",
            iconClass: "fa-solid fa-sink blue px-2",
          },
          {
            name: "Furnished",
            iconClass: "fa-solid fa-couch blue px-2",
          },
        ],
        date: "27/Sep/2021",
        btn: {
          name: "View Details",
          className: "btn btn-primary",
        },
      },
      {
        img: "https://s3-alpha-sig.figma.com/img/9061/1c0b/740cfe7923702cf61e5b5a614f947e22?Expires=1721606400&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=hQqJRWAsdaoiMGAWIjHHyolvgeo5iluApJ6UXMrjgixu2OZtPuGqNr1~IrpKCtAKx9EKJmHs9zZkb2zXB0kyvAgc2aXYAG3UhffaPv0elEshdMswuGMg4IL27NOfer4DGuYfUsXPApztRnukZlrudUfsiIYLVSDXnceHqep2I7XT~rWoylIYMO7ILtHuBmr~fq08zLftPqGYso7oVNYTn5HUzJV4UXY8ZsnFbGDZPHwBOrNXzNvTSBE7grpaNDmrhnQ7~JGWtS09KXbZAeqdcenCqmZwKKpjsq7ll-AoUa3T1Q4mfDzMq066JVJpgJ-OlZxX4vzLEej9T4oqQkSj1A__",
        price: "70 Lac",
        detail: "3 BHK flat in Vijayanagar, Mysuru",
        location:
          "2Nd Floor, Dejgow Building, Kannada Sahithya Parishath Rd, Mysuru - 570017",
        features: [
          {
            name: "2400Sqft",
            iconClass: "fa-regular fa-square blue px-2",
          },
          {
            name: "3 wash",
            iconClass: "fa-solid fa-sink blue px-2",
          },
          {
            name: "Furnished",
            iconClass: "fa-solid fa-couch blue px-2",
          },
        ],
        date: "27/Sep/2021",
        btn: {
          name: "View Details",
          className: "btn btn-primary",
        },
      },
    ],
  },
  office: {
    heading: "Office Spaces in Mysuru",
    SelecteButtons: [{ name: "1200 Sqft" }, { name: "2400 Sqft" }],
    moreBtn: {
      name: "See More",
      iconClass: "fa-solid fa-arrow-right ms-2",
    },
    cards: [
      {
        img: "https://s3-alpha-sig.figma.com/img/9061/1c0b/740cfe7923702cf61e5b5a614f947e22?Expires=1721606400&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=hQqJRWAsdaoiMGAWIjHHyolvgeo5iluApJ6UXMrjgixu2OZtPuGqNr1~IrpKCtAKx9EKJmHs9zZkb2zXB0kyvAgc2aXYAG3UhffaPv0elEshdMswuGMg4IL27NOfer4DGuYfUsXPApztRnukZlrudUfsiIYLVSDXnceHqep2I7XT~rWoylIYMO7ILtHuBmr~fq08zLftPqGYso7oVNYTn5HUzJV4UXY8ZsnFbGDZPHwBOrNXzNvTSBE7grpaNDmrhnQ7~JGWtS09KXbZAeqdcenCqmZwKKpjsq7ll-AoUa3T1Q4mfDzMq066JVJpgJ-OlZxX4vzLEej9T4oqQkSj1A__",
        price: "70 Lac",
        detail: "2 BHK flat in Vijayanagar, Mysuru",
        location:
          "2Nd Floor, Dejgow Building, Kannada Sahithya Parishath Rd, Mysuru - 570017",
        features: [
          {
            name: "2400Sqft",
            iconClass: "fa-regular fa-square blue px-2",
          },
          {
            name: "3 wash",
            iconClass: "fa-solid fa-sink blue px-2",
          },
          {
            name: "Furnished",
            iconClass: "fa-solid fa-couch blue px-2",
          },
        ],
        date: "27/Sep/2021",
        btn: {
          name: "View Details",
          className: "btn btn-primary",
        },
      },
      {
        img: "https://s3-alpha-sig.figma.com/img/9061/1c0b/740cfe7923702cf61e5b5a614f947e22?Expires=1721606400&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=hQqJRWAsdaoiMGAWIjHHyolvgeo5iluApJ6UXMrjgixu2OZtPuGqNr1~IrpKCtAKx9EKJmHs9zZkb2zXB0kyvAgc2aXYAG3UhffaPv0elEshdMswuGMg4IL27NOfer4DGuYfUsXPApztRnukZlrudUfsiIYLVSDXnceHqep2I7XT~rWoylIYMO7ILtHuBmr~fq08zLftPqGYso7oVNYTn5HUzJV4UXY8ZsnFbGDZPHwBOrNXzNvTSBE7grpaNDmrhnQ7~JGWtS09KXbZAeqdcenCqmZwKKpjsq7ll-AoUa3T1Q4mfDzMq066JVJpgJ-OlZxX4vzLEej9T4oqQkSj1A__",
        price: "70 Lac",
        detail: "3 BHK flat in Vijayanagar, Mysuru",
        location:
          "2Nd Floor, Dejgow Building, Kannada Sahithya Parishath Rd, Mysuru - 570017",
        features: [
          {
            name: "2400Sqft",
            iconClass: "fa-regular fa-square blue px-2",
          },
          {
            name: "3 wash",
            iconClass: "fa-solid fa-sink blue px-2",
          },
          {
            name: "Furnished",
            iconClass: "fa-solid fa-couch blue px-2",
          },
        ],
        date: "27/Sep/2021",
        btn: {
          name: "View Details",
          className: "btn btn-primary",
        },
      },
    ],
  },
  shop: {
    heading: "Commercial Shops in Vijayanagara, Mysuru",
    moreBtn: {
      name: "See More",
      iconClass: "fa-solid fa-arrow-right ms-2",
    },
    cards: [
      {
        img: "https://s3-alpha-sig.figma.com/img/9061/1c0b/740cfe7923702cf61e5b5a614f947e22?Expires=1721606400&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=hQqJRWAsdaoiMGAWIjHHyolvgeo5iluApJ6UXMrjgixu2OZtPuGqNr1~IrpKCtAKx9EKJmHs9zZkb2zXB0kyvAgc2aXYAG3UhffaPv0elEshdMswuGMg4IL27NOfer4DGuYfUsXPApztRnukZlrudUfsiIYLVSDXnceHqep2I7XT~rWoylIYMO7ILtHuBmr~fq08zLftPqGYso7oVNYTn5HUzJV4UXY8ZsnFbGDZPHwBOrNXzNvTSBE7grpaNDmrhnQ7~JGWtS09KXbZAeqdcenCqmZwKKpjsq7ll-AoUa3T1Q4mfDzMq066JVJpgJ-OlZxX4vzLEej9T4oqQkSj1A__",
        price: "70 Lac",
        detail: "2 BHK flat in Vijayanagar, Mysuru",
        location:
          "2Nd Floor, Dejgow Building, Kannada Sahithya Parishath Rd, Mysuru - 570017",
        features: [
          {
            name: "2400Sqft",
            iconClass: "fa-regular fa-square blue px-2",
          },
          {
            name: "3 wash",
            iconClass: "fa-solid fa-sink blue px-2",
          },
          {
            name: "Furnished",
            iconClass: "fa-solid fa-couch blue px-2",
          },
        ],
        date: "27/Sep/2021",
        btn: {
          name: "View Details",
          className: "btn btn-primary",
        },
      },
      {
        img: "https://s3-alpha-sig.figma.com/img/9061/1c0b/740cfe7923702cf61e5b5a614f947e22?Expires=1721606400&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=hQqJRWAsdaoiMGAWIjHHyolvgeo5iluApJ6UXMrjgixu2OZtPuGqNr1~IrpKCtAKx9EKJmHs9zZkb2zXB0kyvAgc2aXYAG3UhffaPv0elEshdMswuGMg4IL27NOfer4DGuYfUsXPApztRnukZlrudUfsiIYLVSDXnceHqep2I7XT~rWoylIYMO7ILtHuBmr~fq08zLftPqGYso7oVNYTn5HUzJV4UXY8ZsnFbGDZPHwBOrNXzNvTSBE7grpaNDmrhnQ7~JGWtS09KXbZAeqdcenCqmZwKKpjsq7ll-AoUa3T1Q4mfDzMq066JVJpgJ-OlZxX4vzLEej9T4oqQkSj1A__",
        price: "70 Lac",
        detail: "3 BHK flat in Vijayanagar, Mysuru",
        location:
          "2Nd Floor, Dejgow Building, Kannada Sahithya Parishath Rd, Mysuru - 570017",
        features: [
          {
            name: "2400Sqft",
            iconClass: "fa-regular fa-square blue px-2",
          },
          {
            name: "3 wash",
            iconClass: "fa-solid fa-sink blue px-2",
          },
          {
            name: "Furnished",
            iconClass: "fa-solid fa-couch blue px-2",
          },
        ],
        date: "27/Sep/2021",
        btn: {
          name: "View Details",
          className: "btn btn-primary",
        },
      },
    ],
  },
};

export default data;